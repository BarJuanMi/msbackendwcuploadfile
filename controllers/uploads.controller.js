...
const { actualizarImagenMantenimientos, actualizarImagenPersonal, actualizarImagenProcesos } = require('../helpers/actualizar-imagen');
const fs = require('fs');
...
const fileImageUpload = (req, res = response) => {
    ...
    const tiposValidos = ['hospitales', 'medicos', 'usuarios', 'modelos', 'monitores', 'administrativos', 'empleados', 'vacunados', 'servlavanderia', 'pqrs'];
    ...
    file.mv(uploadPath, (err) => {
        ...
        } else if (tipo === 'vacunados' || tipo === 'servlavanderia' || tipo === 'pqrs') {
...
const fileImageReturn = (req, res = response) => {
    ...
    console.log('FFFFF:::' + req.params.tipo);
    console.log('QQQQQ:::' + req.params.foto);
